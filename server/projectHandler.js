...
async function saveProjectToFile(projectName, projectData) {
    ...
    if (!projectName || typeof projectName !== 'string' || projectName.trim() === '') {
        throw { code: 'INVALID_PROJECT_NAME', message: 'Nama project tidak boleh kosong.' };
    }
    const saneProjectName = projectName.replace(/[^a-z0-9_\-\s\.]/gi, '_').trim();
    if (saneProjectName === '') {
        throw { code: 'INVALID_PROJECT_NAME', message: 'Nama project menjadi tidak valid setelah sanitasi.' };
    }
    ...
    try {
        ...
        throw { code: 'FILE_SAVE_ERROR', message: `Gagal menyimpan file project: ${error.message}`, originalError: error };
    }
}
...
async function loadProjectFromFile(projectName) {
    await ensureProjectsDirExists();
    if (!projectName || typeof projectName !== 'string' || projectName.trim() === '') {
        throw { code: 'INVALID_PROJECT_NAME', message: 'Nama project untuk dimuat tidak valid.' };
    }
    const saneProjectName = projectName.replace(/[^a-z0-9_\-\s\.]/gi, '_').trim();
    const filePath = path.join(PROJECTS_DIR, `${saneProjectName}.json`);
    try {
        ...
        if (error.code === 'ENOENT') {
            ...
            throw { code: 'PROJECT_NOT_FOUND', message: `Project '${saneProjectName}' tidak ditemukan.` };
        }
        console.error(`Gagal memuat project '${saneProjectName}':`, error);
        throw { code: 'FILE_LOAD_ERROR', message: `Gagal memuat file project: ${error.message}`, originalError: error };
    }
}
...
async function listProjectFiles() {
    ...
    try {
        ...
        throw { code: 'LIST_PROJECTS_ERROR', message: `Gagal membaca daftar project: ${error.message}`, originalError: error };
    }
}
