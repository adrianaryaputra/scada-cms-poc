...
function dispatch(path, value, depth) {
    ...
    switch (typeof value) {
        ...
        case T_NUMBER:
            return item(T_NUMBER, path, value, depth);
        case T_BOOLEAN:
            return item(T_BOOLEAN, path, value, depth);
        ...
        default:
            return item(T_UNDEFINED, path, String(value), depth); // undefined and others
    }
}
...
function arr(path, value, depth) {
    ...
    if (value.length === 0) {
        ...
        for (let i = 0; i < value.length; i++) {
            ...
            item_li.appendChild(
                dispatch(subPath, value[i], depth + 1).firstChild,
            );
    ...
    exp.addEventListener("click", function (e) {
        ...
        this.textContent = ol.classList.contains("rdjson-hidden")
            ? `[+] (${value.length})`
            : "[-]";
        e.stopPropagation();
    });
...
function obj(path, value, depth) {
    ...
    exp.addEventListener("click", function (e) {
        ...
        this.textContent = ul.classList.contains("rdjson-hidden")
            ? `{+} (${keys.length})`
            : "{-}";
        e.stopPropagation();
    });
...
renderjson.set_show_to_level = function (level) {
    옵션_show_to_level =
        typeof level === T_STRING && level.toLowerCase() === "all"
            ? Number.MAX_VALUE
            : parseInt(level, 10);
...
renderjson.set_max_string_length = function (length) {
    옵션_max_string_length =
        typeof length === T_STRING && length.toLowerCase() === "all"
            ? 0
            : parseInt(length, 10);
    return renderjson;
};
