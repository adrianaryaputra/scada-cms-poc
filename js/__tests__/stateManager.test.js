...
jest.mock('../projectManager.js', () => ({
    ...
    default: { // Assuming ProjectManager is a default export with an init method and setDirty
        init: jest.fn(),
        setDirty: jest.fn(),
...
describe('StateManager', () => {
    ...
    beforeEach(() => {
        ...
        mockComponentNode = {
            ...
            x: () => 10,
            y: () => 20,
            attrs: { componentType: 'TestComponent', deviceId: 'dev1', variableName: 'var1', label: 'Test' },
            updateState: jest.fn(),
            destroy: jest.fn(),
        };
        ...
        mockComponentFactory = {
            create: jest.fn().mockImplementation((type, data) => ({ ...mockComponentNode, ...data, attrs: {...mockComponentNode.attrs, ...data } })),
        };
        ...
        initStateManager(mockComponentFactory, mockLayer, mockTransformer, mockUndoBtn, mockRedoBtn);
    });
    ...
    describe('saveState', () => {
        beforeEach(() => {
            ...
            getUndoStack().length = 0;
            initStateManager(mockComponentFactory, mockLayer, mockTransformer, mockUndoBtn, mockRedoBtn); // Re-init for a clean start
        });
        ...
        test('should save component data and tags to undoStack', () => {
            ...
            expect(savedState.components[0].componentType).toBe('TestComponent');
    ...
    describe('getCurrentState', () => {
        ...
        test('should handle missing componentType gracefully', () => {
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining("missing componentType"));
    ...
    describe('restoreState', () => {
        ...
        beforeEach(() => {
            stateToRestore = {
                components: [
                    { id: 'compNew', x: 100, y: 150, componentType: 'RestoredComponent', deviceId: 'devR', variableName: 'varR', label: 'Restored' }
                ],
        ...
        test('should clear existing components and recreate from state', () => {
            ...
            expect(mockComponentFactory.create).toHaveBeenCalledWith('RestoredComponent', stateToRestore.components[0]);
        ...
        test('should handle invalid JSON gracefully', () => {
            ...
            expect(consoleErrorSpy).toHaveBeenCalledWith(
                expect.stringContaining("[StateManager] Error parsing or restoring state:"),
        ...
        test('should handle state string with missing components/tags gracefully', () => {
            ...
            expect(consoleErrorSpy).toHaveBeenCalledWith("[StateManager] Invalid state string provided to restoreState:", JSON.stringify({}));
            consoleErrorSpy.mockRestore();
        });
    });
    ...
    describe('Undo/Redo', () => {
        ...
        beforeEach(() => {
            ...
            mockLayer.find.mockReturnValue([{...mockComponentNode, attrs: {...mockComponentNode.attrs}}]); // Return a copy
        ...
        test('handleUndo should restore previous state and move current to redoStack', () => {
            ...
            expect(mockComponentFactory.create).toHaveBeenCalledWith(restoredState1.components[0].componentType, restoredState1.components[0]);
        });
        ...
        test('handleRedo should restore next state and move current to undoStack', () => {
            ...
            expect(mockComponentFactory.create).toHaveBeenCalledWith(restoredState2.components[0].componentType, restoredState2.components[0]);
        ...
        test('handleUndo should not work if only initial state exists', () => {
            ...
            getRedoStack().length = 0;
            initStateManager(mockComponentFactory, mockLayer, mockTransformer, mockUndoBtn, mockRedoBtn); // Only initial state
    ...
    describe('Device Variable Management', () => {
        ...
        test('getDeviceVariableValue should return undefined if not exists', () => {
            expect(getDeviceVariableValue('dev1', 'varNonExistent')).toBeUndefined();
            expect(getDeviceVariableValue('devNonExistent', 'varA')).toBeUndefined();
        });
        ...
        test('setDeviceVariableValue should notify correct component among many', () => {
            const mockComp1 = { attrs: { deviceId: 'd1', variableName: 'v1'}, updateState: jest.fn(), id: ()=>'c1' };
            const mockComp2 = { attrs: { deviceId: 'd1', variableName: 'v2'}, updateState: jest.fn(), id: ()=>'c2' }; // Different var
            const mockComp3 = { attrs: { deviceId: 'd2', variableName: 'v1'}, updateState: jest.fn(), id: ()=>'c3' }; // Different device
        ...
        test('setDeviceVariableValue should call updateLiveVariableValueInManagerUI', () => {
            ...
            expect(updateLiveVariableValueInManagerUI).toHaveBeenCalledWith('dev1', 'var1', 300);
        });
    ...
    describe('Legacy Address-Based Functions', () => {
        ...
        test('getComponentAddressValue should warn and attempt to read from root', () => {
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining("[DEPRECATED] getComponentAddressValue"));
        });

        test('setComponentAddressValue should warn', () => {
            setComponentAddressValue('addr1', 123);
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining("[DEPRECATED] setComponentAddressValue"));
        });
        ...
        test('setComponentAddressValue with deviceId should map to setDeviceVariableValue', () => {
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining("Attempting to map legacy"));
        ...
        test('setComponentAddressValue without deviceId (or _global) should write to root and error log', () => {
            ...
            expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining("[DANGEROUS OPERATION]"));
